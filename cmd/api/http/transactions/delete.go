...
func (r *TransactionsRouter) DeleteByIdRoute() routing.Route {
	...
	return routing.Route{
		...
		Handler: func(c *fiber.Ctx) error {
			...
			if id == "" {
				log.Infof("🔥 Transaction id is required.")
			...
			if err != nil {
				log.Errorf("🔥 Error parsing transaction id: %s", err.Error())
			...
			if err != nil {
				log.Errorf("🔥 Error retrieving transaction: %s", err.Error())
			...
			if existingTransaction.Id == uuid.Nil {
				log.Infof("🔥 Transaction with ID %s not found", id)
