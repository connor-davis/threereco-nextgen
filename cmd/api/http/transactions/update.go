...
func (r *TransactionsRouter) UpdateByIdRoute() routing.Route {
	responses := openapi3.NewResponses()

	responses.Set("200", &openapi3.ResponseRef{
		Value: openapi3.NewResponse().
			WithDescription("The transaction has been successfully updated."),
	})
	...
	return routing.Route{
		OpenAPIMetadata: routing.OpenAPIMetadata{
			...
			Description: "Updates the transaction information for a specific transaction identified by their id.",
			Tags:        []string{"Transactions"},
			Parameters:  parameters,
			RequestBody: bodies.UpdateTransactionPayloadBody,
			Responses:   responses,
		},
		...
		Handler: func(c *fiber.Ctx) error {
			...
			if id == "" {
				log.Infof("🔥 Missing transaction ID in request path")
			...
			if err != nil {
				log.Infof("🔥 Invalid transaction ID format: %v", err)
