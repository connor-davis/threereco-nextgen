...
func (r *MaterialsRouter) UpdateByIdRoute() routing.Route {
	responses := openapi3.NewResponses()

	responses.Set("200", &openapi3.ResponseRef{
		Value: openapi3.NewResponse().
			WithDescription("The material has been successfully updated."),
	})
	...
	return routing.Route{
		OpenAPIMetadata: routing.OpenAPIMetadata{
			...
			Description: "Updates the material information for a specific material identified by their id.",
			Tags:        []string{"Materials"},
			Parameters:  parameters,
			RequestBody: bodies.UpdateMaterialPayloadBody,
			Responses:   responses,
		},
		...
		Handler: func(c *fiber.Ctx) error {
			...
			if id == "" {
				log.Infof("🔥 Missing material ID in request path")
			...
			if err != nil {
				log.Infof("🔥 Invalid material ID format: %v", err)
