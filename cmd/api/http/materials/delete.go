...
func (r *MaterialsRouter) DeleteByIdRoute() routing.Route {
	...
	return routing.Route{
		...
		Handler: func(c *fiber.Ctx) error {
			...
			if id == "" {
				log.Infof("🔥 Material id is required.")
			...
			if err != nil {
				log.Errorf("🔥 Error parsing material id: %s", err.Error())
			...
			if err != nil {
				log.Errorf("🔥 Error retrieving material: %s", err.Error())
			...
			if existingMaterial.Id == uuid.Nil {
				log.Infof("🔥 Material with ID %s not found", id)
