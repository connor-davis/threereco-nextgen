...
func (r *ProductsRouter) DeleteByIdRoute() routing.Route {
	responses := openapi3.NewResponses()

	responses.Set("200", &openapi3.ResponseRef{
		Value: openapi3.NewResponse().
			WithDescription("The product has been successfully deleted."),
	})
	...
	return routing.Route{
		OpenAPIMetadata: routing.OpenAPIMetadata{
			...
			Description: "Deletes a product by by their id.",
		...
		Handler: func(c *fiber.Ctx) error {
			...
			if id == "" {
				log.Infof("🔥 Product id is required.")
			...
			if err != nil {
				log.Errorf("🔥 Error parsing product id: %s", err.Error())
			...
			existingProduct, err := r.Services.Products.GetById(idUUID)

			if err != nil {
				log.Errorf("🔥 Error retrieving product: %s", err.Error())
			...
			if existingProduct.Id == uuid.Nil {
				log.Infof("🔥 Product with ID %s not found", id)
