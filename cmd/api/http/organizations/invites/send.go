...
func (r *OrganizationsInvitesRouter) SendInvite() routing.Route {
	responses := openapi3.NewResponses()

	responses.Set("200", &openapi3.ResponseRef{
		Value: openapi3.NewResponse().
			WithDescription("The invitation has been successfully sent."),
	})
	...
	return routing.Route{
		...
		Handler: func(c *fiber.Ctx) error {
			...
			if currentUser.PrimaryOrganizationId == nil {
				return c.Status(fiber.StatusBadRequest).JSON(&fiber.Map{
					"error":   constants.BadRequestError,
					"message": "You must belong to an organization to send invites.",
				})
			}
