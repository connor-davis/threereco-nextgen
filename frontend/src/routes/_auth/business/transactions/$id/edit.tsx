import {
  deleteApiTransactionsMaterialsByIdMutation,
  postApiTransactionsAssignMaterialByTransactionIdByMaterialIdMutation,
  postApiTransactionsMaterialsMutation,
  putApiTransactionsByIdMutation,
  putApiTransactionsMaterialsByIdMutation,
...
import {
  ...
  type Material,
  type Transaction,
  type TransactionMaterial,
  getApiBusinesses,
  getApiMaterials,
  getApiTransactionsById,
} from '@/api-client';
import {
  zCreateTransactionMaterial,
  zUpdateTransaction,
  zUpdateTransactionMaterial,
...
export const Route = createFileRoute('/_auth/business/transactions/$id/edit')({
  ...
  loader: async ({
    params: { id },
    deps: { businessesSearchTerm, materialsSearchTerm },
  }) => {
    ...
    const { data: transactionResponse } = await getApiTransactionsById({
      client: apiClient,
      path: {
        id,
      },
    ...
    return {
      transaction: (transactionResponse?.item ?? {}) as Transaction,
...
function RouteComponent() {
  ...
  const { transaction, businesses, materials } = Route.useLoaderData();

  const updateTransactionForm = useForm<z.infer<typeof zUpdateTransaction>>({
  ...
  const assignTransactionMaterialMutation = useMutation({
    ...postApiTransactionsAssignMaterialByTransactionIdByMaterialIdMutation({
      client: apiClient,
    }),
    onError: ({ error, message }: ErrorResponse) =>
    ...
    onSuccess: () => {
      toast.success('Success', {
        description: 'The transaction material has been assigned.',
        duration: 2000,
      });
  ...
  const createTransactionMaterialMutation = useMutation({
    ...postApiTransactionsMaterialsMutation({
      client: apiClient,
    }),
    onError: ({ error, message }: ErrorResponse) =>
    ...
    onSuccess: (data) => {
      toast.success('Success', {
        description: 'The transaction material has been added.',
        duration: 2000,
      });

      assignTransactionMaterialMutation.mutate({
        path: {
          transactionId: id,
  ...
  const updateTransactionMaterialMutation = useMutation({
    ...putApiTransactionsMaterialsByIdMutation({
      client: apiClient,
    }),
    onError: ({ error, message }: ErrorResponse) =>
    ...
    onSuccess: () => {
      toast.success('Success', {
        description: 'The transaction material has been updated.',
        duration: 2000,
      });
  ...
  const deleteTransactionMaterialMutation = useMutation({
    ...deleteApiTransactionsMaterialsByIdMutation({
      client: apiClient,
    }),
    onError: ({ error, message }: ErrorResponse) =>
    ...
    onSuccess: () => {
      toast.success('Success', {
        description: 'The transaction material has been deleted.',
        duration: 2000,
      });
  ...
  const materialsTable = useReactTable({
    ...
    data: transaction.materials,
    getCoreRowModel: getCoreRowModel(),
    meta: {
      updateWeight: (id: string, value: number) => {
        updateTransactionMaterialMutation.mutate({
          path: { id },
          body: {
            ...zUpdateTransactionMaterial.parse(
              transaction.materials.find((material) => material.id === id)
            ),
            weight: value,
          },
        });
      ...
      deleteMaterial: (id: string) => {
        deleteTransactionMaterialMutation.mutate({
          path: { id },
        });
      },
    },
    ...
    <>
      <div className="flex w-full h-auto items-center justify-between gap-3">
        <div className="flex w-auto h-auto items-center gap-3">
          ...
          <Label className="text-lg">Update Transaction</Label>
        </div>
                      ...
                      .map((material) => (
                        ...
                        >
                          ...
                          <Button
                            ...
                            className="w-6 h-6"
                            onClick={() =>
                              createTransactionMaterialMutation.mutate({
                                body: zCreateTransactionMaterial.parse({
                                  ...material,
                                  weight: 0.0,
                                  value: 0.0,
                                }),
