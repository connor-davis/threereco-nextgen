import { deleteApiCollectionsByIdMutation } from '@/api-client/@tanstack/react-query.gen';
...
export const Route = createFileRoute('/_auth/admin/collections/')({
  ...
  loader: async ({ deps: { page, pageSize, searchTerm } }) => {
    const { data: collectionsResponse } = await getApiCollections({
      client: apiClient,
      query: {
        page,
        pageSize,
    ...
    if (
      collectionsResponse.pagination !== undefined &&
      page > collectionsResponse.pagination.pages
    ) {
      throw redirect({
    ...
    return {
      collections: collectionsResponse.items as Collection[],
      pagination: collectionsResponse.pagination,
    };
  },
